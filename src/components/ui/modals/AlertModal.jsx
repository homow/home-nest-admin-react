...
export default function AlertModal({message = "", type = "error", isOpen = false, setIsOpen, setData, closeDelay = 5000}) {
    ...
    return (
        <div
            className={cn("text-sm fixed top-3 right-3 max-w-sm w-full flex items-center overflow-hidden justify-center shadow-2xl z-30 sm:text-base", isOpen ? "block" : "hidden")}
            role={"alertdialog"}
            aria-live={"assertive"}
            aria-modal={true}
        ...
        >
            <div className="bg-white dark:bg-gray-800 shadow-xl w-full space-y-2 pt-3">
                ...
                <p
                    className="text-gray-700 px-3 dark:text-gray-300"
                    aria-live={"assertive"}
                >
                    {message}
                </p>
